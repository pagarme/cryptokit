...
func cMechanismList(m []*Mechanism) (arena, C.CK_MECHANISM_PTR, C.CK_ULONG) {
	...
	for i := 0; i < len(m); i++ {
		...
		switch v := m[i].Parameter.(type) {
		case uint:
			pm[i].pParameter = C.CK_VOID_PTR(uintptr(v))
			pm[i].ulParameterLen = C.CK_ULONG(m[i].Length)
		case []byte:
			pm[i].pParameter, pm[i].ulParameterLen = arena.Allocate(v)
		}
